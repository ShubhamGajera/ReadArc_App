...
import 'package:readarc_admin_apk/User/user_storage.dart';
    ...
    if (user != null) {
      DocumentSnapshot<Map<String, dynamic>> userData =
          await FirebaseFirestore.instance
              .collection('users') // Change to the Users collection
              .doc(user.uid)
              .get();
      ...
      if (userData.exists && userData.data() != null) {
        setState(() {
          _username =
              userData.data()!['username']; // Assuming field name is 'username'
  ...
  Future<void> _updateProfile() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        // Call the updateProfile method in AuthService
        String? result = await AuthService().updateProfile(
          uid: user!.uid,
          username: _username!,
          email: _email, // Email is not updated in this case
          profileImagePath: _imageFile != null ? _imageFile!.path : null,
          newPassword: _isChangingPassword
              ? _newPassword
              : null, // Only update if checkbox is checked
        );

        if (result == null) {
          Navigator.pop(context); // Go back to the profile page after updating
        } else {
          // Handle the error, show a message if needed
          print(result);
        }
      } catch (e) {
